...
//
//   Chunk 6 · UTILITIES                canonicalBase()
...
import (
	...
	"dalec-mapping/workflow/infrastructure/test"
...
func resolveTestPaths(item *workplan.WorkItem, osName string, isContainer bool) testPaths {
	...
	tp := testPaths{
		...
		binaryPath: "/usr/bin/" + subject.SpecImageName,
	}
	if isContainer {
		tp.symlinkPath = "/usr/local/bin/" + subject.SpecImageName
	}
